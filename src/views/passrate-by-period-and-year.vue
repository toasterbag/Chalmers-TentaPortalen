...
div(v-if="this.ready")
  .row
    .fs-2 {{ $route.params.start }}/{{ $route.params.end }}
  ...
  .d-flex.justify-content-center(v-if="periods.isEmpty()")
    .fs-3.text-muted No data found for this year
  div(v-else)
  ...
  methods: {
    async load() {
      const res = await Http.get(
        `periods/passrate/${this.$route.params.start}/${this.$route.params.end}`
      );
