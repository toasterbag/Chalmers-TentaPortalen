  ...
  return new CronJob(
    ...
    async function () {
      ...
      const byProgramme = Object.entries(
        data.groupBy((e) => e.course.owner_code),
      )
        .map(
          ([programme_code, surveys]): [
            ...
            return [
              programme_code,
              Object.entries(surveys.groupBy((e) => e.academic_year)).sortBy(
      ...
      for (const [programme, years] of Object.entries(byProgramme)) {
        await ctx.redis_cache.programme_survey_aggregate.set(
          programme,
