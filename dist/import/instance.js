...
exports.scrape_instances_for_id = void 0;
...
const scrape_instances_for_id = async (course_code, study_portal_id) => {
    if (study_portal_id === error_1.ERROR_TYPES.COURSE_INSTANCE_NOT_FOUND) {
        return error_1.ERROR_TYPES.COURSE_INSTANCE_NOT_FOUND;
    }
    //const url = `https://www.student.chalmers.se/sp/print_course?course_id=${study_portal_id}`;
    let result = [];
    try {
        const [res_sv, res_en] = await Promise.all([
            node_fetch_1.default(`https://student.portal.chalmers.se/sv/chalmersstudier/minkursinformation/Sidor/SokKurs.aspx?course_id=${study_portal_id}&parsergrp=3`, { timeout: 30_000 + Math.random() * 5000 }),
            node_fetch_1.default(`https://student.portal.chalmers.se/en/chalmersstudies/courseinformation/Pages/SearchCourse.aspx?course_id=${study_portal_id}&parsergrp=3`, {
        ...
        if (e.type == "request-timeout") {
            Log.warn(`Timeout requesting the page for '${course_code}' instance ${study_portal_id}. Retrying...`);
            return scrape_instances_for_id(course_code, study_portal_id);
        }
    }
    const [html_sv, html_en] = result;
    const $ = cheerio_1.default.load(html_sv);
    const $en = cheerio_1.default.load(html_en);
    const instances = [];
    ...
    $("table table").each((i, e) => {
        const is_study_period_table = $(e).find("td > img[src='https://www.student.chalmers.se/sp/images/ico_info.gif']");
        if (is_study_period_table.length) {
            const instance = new Instance({
                course_code,
                study_portal_id,
            });
            const is_sp1 = $(e).find("tr:nth-child(n+3) td:nth-child(6)").html().trim().length > 0;
            const is_sp2 = $(e).find("tr:nth-child(n+3) td:nth-child(7)").html().trim().length > 0;
            const is_sp3 = $(e).find("tr:nth-child(n+3) td:nth-child(8)").html().trim().length > 0;
            const is_sp4 = $(e).find("tr:nth-child(n+3) td:nth-child(9)").html().trim().length > 0;
            const is_summer = $(e).find("tr:nth-child(n+3) td:nth-child(10)").html().trim().length >
                0;
...
exports.scrape_instances_for_id = scrape_instances_for_id;
