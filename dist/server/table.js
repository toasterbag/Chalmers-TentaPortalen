...
const strip_formatting = (s) => s.replace(ANSI_REGEX, "");
const width = () => process.stdout.columns - 12;
const len = (s) => strip_formatting(s).length;
const gen_header = (text) => {
...
const print_table = (header_text, columns, data) => {
    const col_width = columns.reduce((acc, col) => {
        acc[col] = len(col);
        return acc;
    }, {});
