...
export default defineComponent({
  ...
  async setup() {
    ...
    const validateQueryParam = (p: LocationQueryValue | LocationQueryValue[]): LocationQueryValue => !Array.isArray(p) ? p : null

    const q = reactive({
      academicYear: validateQueryParam(route.query.academic_year) ?? availableYears[1],
      ...
      minParticipants: Number(validateQueryParam(route.query.min_participants) ?? 0),
      maxParticipants: Number(validateQueryParam(route.query.max_participants) ?? 500),
    });

    const sortingKey = ref(String(route.query.sorting_key ?? "passrate"));
    const orderDescending = ref(Boolean(route.query.order_descending ?? "false"));

    let loading = ref(false);
    let timer = setTimeout(() => { }, 0);
    let courses: Ref<Array<Exam & { passrate: number, participants: number }>> = ref([])
    ...
    const sortCourses = () => {
      ...
      setTimeout(() => {
        courses.value = courses.value
          .filter((e) => e.participants >= q.minParticipants && e.participants <= q.maxParticipants)
          .sortBy((a: any, b: any) => {
