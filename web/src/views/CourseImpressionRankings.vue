    ...
    .row.justify-content-center.tenta-table
      ...
      .col-12
        .row.header.align-items-center.user-select-none
          .col-3 Code
          .col-2.text-end.clickable(@click="setSortingKey('total_impression_mean')")
...
export default defineComponent({
  ...
  async setup() {
    ...
    const availableYears = new Array(8)
      ...
      .map((year) => date_to_academic_year(new Date(year, getMonth(new Date()))));
    ...
    const q = reactive({
      ...
      maxResponses: Number(validateQueryParam(route.query.max_responses) ?? 500),
    });

    const sortingKey = ref(String(route.query.sorting_key ?? "total_impression_mean"));
